    ...
    if ($query->num_rows() > 0) {
        ...
        foreach ($query->result() as $row) {
            ...
            $result[] = [
                ...
                "date" => $row->date,
                "details" => $row->details,
                "blog_status" => $row->blog_status ?? 'published',
                "slug" => $row->slug ?? '',
...
public function get_by_slug_blog()
{
    $this->_init_cors();

    $input = json_decode(file_get_contents("php://input"), true);
    $slug = $input['slug'] ?? $this->input->post('slug');
   ...
   public function delete_blog()
   {
    ...
    if ($status !== null) $updateData['blog_status'] = $status;
...
public function update_consultation()
{
    ...
    if (empty($input['page_name']) || empty($input['meta_title'])) {
        echo json_encode([
            "status" => "false",
            "message" => "Page name and Meta Title are required"
        ]);
        return;
    }
...
public function get_id_seo()
{
    ...
    if (empty($input['id']) || empty($input['page_name']) || empty($input['meta_title'])) {
        echo json_encode([
            "status" => false,
            "message" => "ID, Page Name, and Meta Title are required"
        ]);
        return;
    }
...
public function get_seo_by_page()
{
    ...
    public function upload_blog_image()
    {
        ...
        if (move_uploaded_file($file['tmp_name'], $filepath)) {
            ...
            $file_url = $scheme . '://' . $host . $base_path . '/uploads/blog_image/' . $filename;
            echo json_encode([
                "status" => "true",
                "url" => $file_url
            ]);
